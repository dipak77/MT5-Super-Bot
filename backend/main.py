...
async def mt5_keepalive_loop():
    """Background loop to auto-detect when desktop MT5 terminal is opened"""
    while True:
        try:
            await asyncio.sleep(25)
            if not trader.connected:
                await asyncio.to_thread(trader.connect)
...
async def lifespan(app: FastAPI):
    # Startup
    trader.connect()
    keepalive = asyncio.create_task(mt5_keepalive_loop())
    yield
    # Shutdown
    keepalive.cancel()
    print("Shutting down...")

app = FastAPI(title="MT5 Super Bot - Institutional Terminal", version="2.0", lifespan=lifespan)
...
def api_status():
    ...
    return {
        "message": "MT5 Super Bot Institutional Terminal Running",
        "version": "2.0",
        "symbols": config.SYMBOLS,
        "timeframes": config.TIMEFRAMES,
        "connected": trader.connected,
        "mode": acc.get("mode", "UNKNOWN"),
        "connection_detail": acc.get("connection_detail", ""),
...
@app.post("/api/account/reconnect")
async def reconnect_mt5():
    """Manual trigger to attempt real IPC connection to desktop MT5"""
    ok = await asyncio.to_thread(trader.connect)
    acc = trader.get_account_info()
    return {"connected": ok, "account": acc}
